...
int numberOfRemovedRollsThisIteration;
char[,] updatedMatrix;
do {
    numberOfRemovedRollsThisIteration = 0;
    updatedMatrix = (char[,])matrix.Clone();
        ...
        for(int j=0; j<arrayLength; j++)
        {
            if (matrix[i,j] == '@')
            {
                if (FindAdjacentRolls(i,j,matrix,arrayLength) < 4)
                {
                    ...
                    updatedMatrix[i,j] = '.';
                    numberOfRemovedRollsThisIteration++;
                }   
            }
        }
    }
    matrix = (char[,])updatedMatrix.Clone();
} while (numberOfRemovedRollsThisIteration > 0);
